...
class StartGatewayPayment
{
    ...
    public function execute(Invoice $invoice, User $actor): StartGatewayPaymentResult
    {
        ...
        return $this->start($invoice, $actor);
    }
	...
	Invoice $invoice,
	?User $actor=null
	): StartGatewayPaymentResult
    {
        $this->reconcileStaleAttempt($invoice, $actor);
        $state = DB::transaction(function () use ($invoice, $actor) {
   ...
   private function reconcileStaleAttempt(
    	Invoice $invoice,
    	?User $actor = null
    ): void {
    	$staleBefore = now()->subMinutes(5);
    ...
    $attempt = $invoice->paymentAttempts()
        ...
        ->reconciliationCandidate($staleBefore, $now)
        ->latest('id')
        ->first();
